...
*   **輸入原始 PPT：** 系統預設會自動帶入您在步驟 1 上傳的 PowerPoint 檔案。如果網頁記憶體已被清除（例如重新整理過網頁），您可以手動在此處重新上傳 `.pptx` 檔案。
*   **輸入審核 Excel：** 系統會自動帶入步驟 4 產生的 AI 最終審核報告。如果您選擇跳過網頁 AI 驗證（例如使用免費的 Excel Copilot 方案），請在此處上傳您手動完成的 Excel 檔案。*注意：上傳的 Excel 必須包含精確命名的 `Status` 與 `Result` 欄位。*
...
*   **支援平台：** Instagram, Threads, 小紅書 (Xiaohongshu), WeChat, HK01。
...
## 5. 計劃中的改進與構想 
* **1. 步驟 1 是否可以將簡報表格（Table）內的超連結也一併萃取出來？**
  * *構想：* 評估升級 PPT 萃取邏輯，以掃描並提取隱藏在表格中的超連結，確保不遺漏任何參考資料。
* **2. 步驟 3 是否能夠支援更多社群平台的網頁爬蟲？**
  * *構想：* 擴大爬蟲功能以支援更多樣化的平台。
* **3. 步驟 5 產出的 PPT 是否能與步驟 4 的 Excel 審核報告確保達到 100% 完全相符？**
* **4. 是否能將所有步驟合併為單一的「一鍵完成」流程？**
